...
function OrderCanceledScreen({ navigation }) {
    ...
    const unsubscribe = navigation.addListener("focus", () => {
      const getData = async () => {
        try {
          ...
          setLoading(true)
          axios.get(url + "/api/transaction/order-canceled", { params: data }).then(res => {
    ...
    <ScrollView>
      {transactions.length < 1 ? (
        ...
        transactions.map((data, key) => {
          return (
            <OrderList key={key} index={key} onPress={() => navigation.navigate("OrderDetail", {screen: 'OrderDetailScreen', params: {id: data.transaction_id, status: 5}})} id={data.transaction_id}
  ...
  return (
    <View style={styles.list}>
      <View style={styles.listHeader}>
        <Text>#{props.id}</Text>
        <Text style={{ color: "tomato" }}>Gagal</Text>
      </View>

...
const DetailScreen = ({ route }) => {
  ...
  const addWishlist = async () => {
    try {
      ...
      axios.get("http://10.0.2.2:8000/api/wishlist/" + product_id, { params: data }).then(res => {
  ...
  const removeWishlist = async () => {
    try {
      ...
      axios.get("http://10.0.2.2:8000/api/r-wishlist/" + product_id, { params: data }).then(res => {
        setWishlist(res.data.status);
      });
    } catch (e) {
      console.warn(e.message);
